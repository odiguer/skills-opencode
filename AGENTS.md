...
- `in-progress/`: beta: public on purpose, feedback wanted, not shipped in the default set
- `deprecated/`: no longer used

Every skill in `engineering/` or `productivity/` (the **promoted** buckets) must have a reference in the top-level `README.md` and an entry in `skills-manifest.json`'s `skills` array (`scripts/install-opencode.sh` installs exactly the promoted set). Skills in `misc/`, `in-progress/`, and `deprecated/` must not appear in either.
...
To (re)link the promoted skills into `.opencode/skills/` for local opencode development, run `scripts/bootstrap-opencode.sh`. Each entry is a symlink into this repo, so a `git pull` keeps installed skills current; re-run the script after adding, removing, or renaming a skill in `skills-manifest.json`.
