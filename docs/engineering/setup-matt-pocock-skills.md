...
It is a prompt-driven skill, not a deterministic script. It reads your `git remote`, your existing `AGENTS.md`, your `opencode.json`, your existing `CONTEXT.md`, proposes what it found, and waits for you to confirm before writing anything.
...
| An `## Agent skills` block | `AGENTS.md`, created at the repo root if missing |
...
By design in this fork: the `## Agent skills` block always goes to `AGENTS.md`, created at the repo root if it doesn't exist, because opencode reads `AGENTS.md`. A `CLAUDE.md` left over from another [harness](https://www.aihero.dev/ai-coding-dictionary/harness) is left untouched. If `opencode.json` is missing as well, the skill won't write opencode config itself: it tells you to run the repo's install script, or add `.opencode/skills` to `skills.paths` by hand, before the engineering skills can load.
...
**Can I keep the config globally instead of committing it to every repo?**
