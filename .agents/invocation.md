...
Every skill also carries an `agents/openai.yaml` beside its `SKILL.md`. It holds Codex UI metadata: `interface.display_name` and `interface.short_description` for the skill picker, and, for user-invoked skills, the `policy.allow_implicit_invocation: false` that pairs with `disable-model-invocation`. Keep the two in sync: a skill is user-invoked in both harnesses or neither. (This fork runs under opencode, which honours `disable-model-invocation` from the frontmatter; the `agents/openai.yaml` files are carried from upstream for Codex compatibility.)
